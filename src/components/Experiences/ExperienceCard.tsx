...
const workExperiences = [
  {
    ...
    company: "IT Kyujin Nabi, Crowdworks",
...
export default function WorkExperienceCard() {
  ...
  const handleAvatarClick = (expId, avatarsLength) => {
    ...
    setCurrentAvatarIndex((prev) => ({
      ...prev,
      [expId]:
        (prev[expId] || 0) + 1 >= avatarsLength ? 0 : (prev[expId] || 0) + 1,
    }));
  };
  ...
  return (
    <div className="w-full my-4 rounded-xl border border-[#E0DFDC] bg-white shadow-sm p-4">
      {workExperiences.map((exp) => (
        <div
          key={exp.id}
          className="border-b border-[#F4F4F4] p-4 last:border-0"
        >
          <div className="flex gap-8">
            ...
            <div className="w-full">
              ...
              <div className="mt-0.5 flex flex-wrap text-[0.9rem] text-[#181818]">
                <span className="mr-2 font-semibold text-[#0077B5]">
                  {exp.company}
                </span>
